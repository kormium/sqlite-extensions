...
/**
 * SQLite's own `ext/misc/regexp.c` — the `REGEXP` operator and a `regexp()` function — packaged as a Kormium extension.
 *
 * ```kotlin
 * val db = createSqliteDatabase("app.db") {
 *     sqlite { extension(SqliteRegexp) }
 * }
 * ```
 *
...
public expect object SqliteRegexp : SqliteExtension {
