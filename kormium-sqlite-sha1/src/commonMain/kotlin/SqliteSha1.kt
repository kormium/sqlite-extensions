...
/**
 * SQLite's own `ext/misc/sha1.c` — `sha1()` and `sha1_query()` hashes — packaged as a Kormium extension.
 *
 * ```kotlin
 * val db = createSqliteDatabase("app.db") {
 *     sqlite { extension(SqliteSha1) }
 * }
 * ```
 *
...
public expect object SqliteSha1 : SqliteExtension {
